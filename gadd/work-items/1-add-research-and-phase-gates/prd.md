---
work item: 1
title: "Add GADD research and phase input gates"
created: 2026-05-14
updated: 2026-05-14
status: approved
---

# PRD: Add GADD research and phase input gates
...
GADD can currently create plausible PRD drafts from thin context, but it does not have a first-class way to decide whether the source inputs are good enough before scoping starts. This creates two product risks for maintainers and product authors:
...
The problem is sharper now that GADD is being configured for GitHub projection. External visibility is useful for review and collaboration, but only if GADD validates input quality, preserves repo-local canonical state, and keeps private PM intake material out of shareable artifacts.

## Goals

- Add a first-class GADD research phase that gathers PM-grade inputs before PRD scoping when the source context is weak, sensitive, or needs codebase investigation.
...
- Add input-quality gates across GADD phases so each command validates that its approved or source inputs meet that phase's standard before producing or mutating its artifact.
- Strengthen `/gadd:scope` so it checks whether supplied inputs are good enough to define a useful product boundary, and routes to research or asks for missing source context when they are not.
...
- Do not make GitHub the canonical source of GADD workflow state.
- Do not mutate GitHub from `/gadd:scope`; external tracker mutation remains gated by explicit human confirmation and the owning GADD command.
...
- GADD maintainer - needs each command to reject weak upstream inputs instead of silently inventing missing context.
...
- GitHub reviewer - needs enough shareable context to review GADD work without seeing private financial or commercial source material.

## User Stories

1. As a GADD maintainer, I want a research phase before scoping when inputs are weak or sensitive, so that PRD scope is based on explicit evidence rather than agent invention.
...
3. As a product author handling sensitive inputs, I want private PM context to remain outside committed and GitHub-visible artifacts, so that financial or commercial detail does not leak during GADD workflow projection.
4. As a GADD maintainer, I want each phase to validate its inputs before writing its artifact, so that bad upstream artifacts are rejected and routed to the earliest phase that can fix them.
5. As a GitHub reviewer, I want projected GADD artifacts to be sanitized and clearly tied back to repo-local canonical state, so that GitHub review validates integration without becoming the workflow source of truth.
...
- [ ] A maintainer can run research with full read-only visibility into repository, documentation, existing GADD artifacts, and private/local context supplied by the human.
...
- [ ] Every GADD phase states the required input standard for that phase before writing or mutating its artifact.
- [ ] When a phase rejects its inputs, it names the blocking input gap and recommends the earliest GADD command that can fix the gap.
...
- Research readiness clarity: 100% of research outputs include one explicit readiness label and a recommended next GADD command or stop reason.
...
- Phase routing clarity: 100% of phase-gate rejections name the failing input standard and the earliest GADD command that can repair it.
...
- `/gadd:research` needs a clear sensitivity boundary between private intake material and shareable repo/GitHub artifacts.
- Existing GADD command boundaries must be preserved: PM commands own product inputs and scope, design owns engineering translation, plan owns executable slices, implementation owns child work, verification and close own closure readiness and closure.
...
- Non-blocking, owner engineering design: Decide the implementation order for applying phase input gates across all GADD commands.
