...
- Compatibility constraints:
  ...
  - Local tracker mode continues to use local IDs; GitHub tracker mode continues to use GitHub issue numbers as approved Work Item IDs.
- Explicit non-goals:
...
Research output is `research.md` in the draft or promoted Work Item directory. Add `skills/gadd-setup/assets/templates/research.md`, and install it as `gadd/templates/research.md`. The template should include:
...
artifacts:
  research:
    path: "gadd/work-items/_drafts/<draft-id>/research.md"
...
| `/gadd:implement` | Ready child Work Item with approved parent PRD, SDD, and plan boundaries. | `/gadd:decompose` or parent planning/design command. |
| `/gadd:verify` | Implemented child Work Item plus explicit verification scope and approved parent boundaries. | `/gadd:implement` or owning parent command for drift. |
...
- PRD approval in GitHub tracker mode uses the GitHub issue number as the Work Item ID.
...
| --- | --- | --- | --- |
| `/gadd:research [new|draft-id|work-item-id] [context]` | Skill package | Human/agent adapters | New command; no existing callers break. |
