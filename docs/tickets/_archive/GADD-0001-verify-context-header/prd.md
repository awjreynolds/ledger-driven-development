...
Implementation closure is also underspecified. `/gadd:implement` can produce code and tests, but GADD does not yet have a separate closure gate that decides whether a child ticket is verified, ready to archive, and safe to close externally.
...
2. As an engineering reviewer, I want implementation work to pass a dedicated GADD verification gate before closure, so that code is not treated as done until evidence is checked against the child ticket, PRD, SDD, and plan.
