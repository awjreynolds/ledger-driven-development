...
Update the workflow command contracts so `/gadd:next` reports the next command and next human action, and PRD/SDD review gates route to `/gadd:approve <ticket-id>`.
...
- `/gadd:next` names `/gadd:approve <ticket-id>` for PRD and SDD approval gates.
...
- Ledger: `docs/tickets/_archive/GADD-0003-002-next-and-approval-routing/ledger.yml`
