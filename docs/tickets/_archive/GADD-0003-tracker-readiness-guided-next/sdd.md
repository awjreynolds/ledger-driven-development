---
ticket: GADD-0003
prd: docs/tickets/_archive/GADD-0003-tracker-readiness-guided-next/prd.md
created: 2026-05-13
updated: 2026-05-13
status: approved
adrs: []
...
- Product constraints from the PRD:
  ...
  - PRD and SDD approval must use explicit `/gadd:approve <ticket-id>` rather than conversation-only shorthand.
...
- `/gadd:next`
  ...
  - For PRD/SDD approval gates, reports `/gadd:approve <ticket-id>`.
...
artifacts:
  prd:
    path: docs/tickets/_archive/GADD-0003-tracker-readiness-guided-next/prd.md
    status: approved
  sdd:
    path: docs/tickets/_archive/GADD-0003-tracker-readiness-guided-next/sdd.md
    status: draft
...
- `/gadd:refine` and `/gadd:design` may still ask the human whether the artifact is ready, but their next durable approval action should name `/gadd:approve <ticket-id>`.
...
- Existing commands that mention "approve PRD" remain understandable, but new prompt text should prefer `/gadd:approve <ticket-id>`.
...
1. PRD approval path:
   1. `/gadd:refine <ticket>` finishes the PRD and sets `current_gate: prd_approval`.
   2. `/gadd:next <ticket>` reports `/gadd:approve <ticket>` as the next human action.
   3. `/gadd:approve <ticket>` sees exactly one PRD approval gate.
   4. It marks PRD frontmatter and ledger artifact status as approved.
   5. It sets `approved_artifacts.prd`, records `prd_approved`, and routes to `/gadd:design <ticket>`.

2. SDD approval path:
   1. `/gadd:design <ticket>` writes `sdd.md` and sets `artifacts.sdd.status: draft`.
   2. It sets `current_gate: design_review` and `next_human_action: /gadd:approve <ticket>`.
   3. `/gadd:approve <ticket>` sees exactly one SDD approval gate.
   4. It marks SDD frontmatter and ledger artifact status as approved.
   5. It sets `approved_artifacts.sdd`, records `sdd_approved`, and routes to `/gadd:plan <ticket>`.
...
| --- | --- | --- | --- |
| `/gadd:approve [ticket-id]` | New `gadd-approve` skill | Maintainers, `/gadd:next`, host adapters | Infers PRD vs SDD only when one approval gate is active. |
...
| `execution_context.next_human_action` | Mutating GADD commands | `/gadd:next`, maintainers | Should name `/gadd:approve <ticket-id>` for PRD/SDD gates. |
