...
sdd: docs/tickets/_archive/GADD-0003-tracker-readiness-guided-next/sdd.md
created: 2026-05-13
updated: 2026-05-13
status: approved
---
...
- Explicit `/gadd:approve <ticket-id>` for PRD and SDD approval.
...
- `/gadd:approve <ticket-id>` can infer PRD vs SDD approval when exactly one gate is active.
...
- PRD and SDD approval gates name `/gadd:approve <ticket-id>`.
...
| `/gadd:approve <ticket-id>` PRD/SDD approval | Slice 1 and Slice 2 |
...
- Use local grep-based checks in the validation script to verify:
  ...
  - `/gadd:next` names `next_human_action` and `/gadd:approve <ticket-id>` for approval gates.
