...
# Software Design Document: Make LDD ready for external trackers and guided next actions
...
- Product constraints from the PRD:
  ...
  - `/ldd:next` must diagnose state before offering continuation.
  - PRD and SDD approval must use explicit `/ldd:approve <ticket-id>` rather than conversation-only shorthand.
  - `/ldd:approve` does not approve plans, decomposition, closure, or external mutations in this PRD.
- Technical constraints from existing code:
  ...
  - Existing PRD/SDD/plan/verification templates are copied into target repos by `/ldd:setup`.
- Operational constraints:
  ...
  - `/ldd:next` must remain safe to run repeatedly.
- Compatibility constraints:
  - Existing local-only ledgers remain valid.
  - Existing approved PRDs and SDDs continue to work without `/ldd:approve` history.
  - Existing `approve PRD` prompt wording can be supported as legacy guidance, but the forward path should name `/ldd:approve`.
- Explicit non-goals:
  ...
  - No plan, decomposition, closure, or external mutation approval in `/ldd:approve`.
  - No hidden automation of durable state changes from `/ldd:next`.

## Existing System

- Current flow:
  - `/ldd:scope`, `/ldd:elaborate`, and `/ldd:refine` create and approve PRDs.
  - `/ldd:design` creates SDDs.
  - `/ldd:plan` creates implementation plans and plan review artifacts.
  - `/ldd:decompose`, `/ldd:implement`, `/ldd:verify`, and `/ldd:close` advance child work.
...
- Extension points:
  - Additive command-shaped skills can be introduced by following the existing `ldd-verify` and `ldd-close` pattern.
  - Existing templates can be extended without breaking current local ledgers.
  - `.ldd/config.yml` already has `tracker.provider`, `repo`, `project`, and branch/PR title config fields.
- Fragile or risky areas:
  ...
  - `/ldd:next` is read-only and safe, but it does not yet clearly distinguish "next command" from "next human action".
  - External tracker synchronization can easily become a broad sync engine if not constrained.
- Prior art to follow:
  - `/ldd:verify` was added as a standalone command for a distinct gate.
...
| --- | --- | --- |
| Add `/ldd:approve` as a standalone command-shaped skill. | Approval needs to be durable and understandable outside the current conversation. A command matches existing package architecture. | PRD, existing command-skill pattern |
| Limit `/ldd:approve` to PRD and SDD approval for this PRD. | The PRD explicitly excludes plan, decomposition, closure, and external mutation approval. | PRD |
...
| Update `/ldd:next` to report both `next_command` and `next_human_action`, and offer safe continuation text without mutating state itself. | The PRD wants continuation from diagnosis, while preserving safety and explicit human confirmation. | PRD, existing `/ldd:next` read-only rule |
| Treat GitHub-first visibility as managed projections of existing LDD artifacts, not as canonical state. | The repo-local ledger remains canonical and the PRD rejects a full sync engine. | PRD, existing templates |
...
| Require `/ldd:approve prd LDD-0003` and `/ldd:approve sdd LDD-0003`. | It is explicit but noisier than needed when ledger state is unambiguous. | Inference is default; explicit artifact targets can be follow-on if ambiguity appears. |
| Make `/ldd:next` execute the next command automatically. | It would make a read-only diagnostic command mutate durable state. | `/ldd:next` offers the next action but waits for a separate command or explicit user instruction. |
...
- `/ldd:approve`
  - New standalone command-shaped skill under `skills/ldd-approve/`.
...
- `/ldd:next`
  ...
  - For PRD/SDD approval gates, reports `/ldd:approve <ticket-id>`.
  - Offers safe continuation phrasing, for example: `Next action: /ldd:approve LDD-0003. Run that when ready.`
  - Does not itself perform approval, external mutation, archival, or artifact writes.

- GitHub-first tracker visibility
  - Extends command contracts for PM/design/plan/decompose/implement/verify/close to use GitHub only when `.ldd/config.yml` has `tracker.provider: github` and required repo configuration.
...
- PM shared-understanding gates
  - Keep the recently added bounded gates in `/ldd:scope`, `/ldd:elaborate`, and `/ldd:refine`.
  - Extend validation to require key phrases so regressions are caught.

- Package surface
  - Add `approve` to `scripts/validate-ldd-mvp.sh` command list.
...
No new top-level ledger section is required. `/ldd:approve` uses existing fields:
...
execution_context:
  ...
  next_command: /ldd:approve LDD-0003
...
- `/ldd:approve` becomes the only LDD command for PRD and SDD approval.
- `/ldd:refine` and `/ldd:design` may still ask the human whether the artifact is ready, but their next durable approval action should name `/ldd:approve <ticket-id>`.
...
- Ambiguous approval gate: `/ldd:approve` stops and reports the candidate gates plus the explicit next command needed to disambiguate.
- Missing artifact: `/ldd:approve` stops and routes to the command that should create it.
- Artifact not ready: `/ldd:approve` stops and routes to `/ldd:refine` for PRD or `/ldd:design` for SDD.
...
- Existing commands that mention "approve PRD" remain understandable, but new prompt text should prefer `/ldd:approve <ticket-id>`.
- Local-only mode continues to work without GitHub.
- Ledgers without `next_human_action` can still be interpreted by `/ldd:next` using artifact statuses.

## Data Flow / Control Flow

1. PRD approval path:
   1. `/ldd:refine <ticket>` finishes the PRD and sets `current_gate: prd_approval`.
   2. `/ldd:next <ticket>` reports `/ldd:approve <ticket>` as the next human action.
   3. `/ldd:approve <ticket>` sees exactly one PRD approval gate.
   4. It marks PRD frontmatter and ledger artifact status as approved.
   5. It sets `approved_artifacts.prd`, records `prd_approved`, and routes to `/ldd:design <ticket>`.

2. SDD approval path:
   1. `/ldd:design <ticket>` writes `sdd.md` and sets `artifacts.sdd.status: draft`.
   2. It sets `current_gate: design_review` and `next_human_action: /ldd:approve <ticket>`.
   3. `/ldd:approve <ticket>` sees exactly one SDD approval gate.
   4. It marks SDD frontmatter and ledger artifact status as approved.
   5. It sets `approved_artifacts.sdd`, records `sdd_approved`, and routes to `/ldd:plan <ticket>`.

3. `/ldd:next` continuation path:
...
5. GitHub SDD/plan review path:
   1. `/ldd:design` and `/ldd:plan` prepare or update a review PR body from `pr-body-sdd-plan.md`.
   2. Branch naming uses existing `.ldd/config.yml` branch templates.
...
| --- | --- | --- | --- |
| `/ldd:approve [ticket-id]` | New `ldd-approve` skill | Maintainers, `/ldd:next`, host adapters | Infers PRD vs SDD only when one approval gate is active. |
| `commands/ldd/approve.md` and `.toml` | LDD package | Claude/Gemini command routers | Thin adapters like existing commands. |
| `skills/ldd-approve/agents/openai.yaml` | LDD package | Codex skill UI | Follows existing metadata shape. |
| `execution_context.next_human_action` | Mutating LDD commands | `/ldd:next`, maintainers | Should name `/ldd:approve <ticket-id>` for PRD/SDD gates. |
...
| GitHub SDD/plan PR body | `/ldd:design` and `/ldd:plan` | GitHub reviewers | Existing `pr-body-sdd-plan.md`. |
...
- Rollout:
  - Add `/ldd:approve` package surface.
  - Update `/ldd:next`, `/ldd:refine`, and `/ldd:design` to route approval gates to `/ldd:approve`.
  - Update setup templates and docs for new command lists.
  - Add validation checks for approve command and bounded gates.
- Backout:
  - Remove `/ldd:approve` from manifests/adapters/validation.
  - Restore `/ldd:refine` and `/ldd:design` prompt wording to direct approval text.
...
- Compatibility tests:
  - Existing `LDD-0001` and archived `LDD-0002` ledgers remain readable by `/ldd:next`.
  - `LDD-0003` approval path works with `/ldd:approve LDD-0003`.
...
| --- | --- | --- | --- |
| Should `/ldd:approve` accept explicit artifact arguments in addition to inference? | Could help ambiguous edge cases, but not needed for the approved PRD. | Engineering design | Treat as follow-on unless implementation finds unavoidable ambiguity. |
...
These questions do not block planning because the first implementation can use inference-only `/ldd:approve` and existing tracker fields, adding fields only if required by the GitHub slice.
