...
| `/ldd:design` | Technical Design | Create or update the repo-scoped Software Design Document (SDD). | approved PRD, code/docs, Architecture Decision Records, related repo context, optional GitNexus context | draft `sdd.md`, ADR updates only when the ADR threshold is met | `/ldd:approve <ticket-id>` |
