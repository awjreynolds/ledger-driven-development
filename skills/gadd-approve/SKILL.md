...
2. If tracker mode is GitHub and the Work Item is still in a draft directory:
   ...
   - Apply additive GitHub labels from config, including the product requirement type label and current phase label when configured.
   - Ensure the projected issue keeps the template's Next Action, Reviewer Focus, and GADD Traceability sections so a non-GADD engineer or agent can continue from the ticket plus repo artifacts.
...
4. In GitHub tracker mode, create or bind the GitHub SDD issue before marking the SDD approved:
   ...
   - Apply additive GitHub labels from config, including the engineering change type label and current phase label when configured.
   - Ensure the projected issue keeps the template's Next Action, Reviewer Focus, and GADD Traceability sections so a non-GADD engineer or agent can continue from the ticket plus repo artifacts.
