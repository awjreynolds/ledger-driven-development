---
name: gadd-verify
description: Run /gadd:verify for an implemented GADD child ticket. Use when the user says /gadd:verify or wants to verify child-ticket closure readiness after implementation completion.
---
...
If inputs fail this standard, write only a failed or override-required verification report when enough child context exists; otherwise stop without mutation. The earliest GADD command that can repair missing implementation evidence is `/gadd:implement`; parent artifact drift routes to the owning `/gadd:scope`, `/gadd:design`, or `/gadd:plan` command.
...
- implementation evidence from the child ledger, local diff summary, commit/PR reference, or implementation notes recorded by `/gadd:implement`
...
- If the PR is closed without merge, verification status must be `override_required`; next human action is reconcile the implementation path or return to `/gadd:implement <child-ticket-id>`.
